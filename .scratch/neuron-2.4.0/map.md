...
- **Ticket 37 resolved, 2026-08-12** (see its own Decisions-so-far entry
  above) — no Notes bullet was appended at the time, so this map's own
  narrative went stale for one session; caught by scanning the issue files
  directly rather than trusting the last-stated frontier (same class of gap
  `13`'s resolution flagged earlier on this same map). Resolving `37` also
  chartered [39](issues/39-config-autodeclare-escapes-projectroot.md)
  (unclaimed, unblocked) as a byproduct bug find, not a graduation.
- **Ticket 25 resolved, 2026-08-12.** Picked up as the frontier's
  lowest-numbered unclaimed-and-unblocked ticket per a direct scan of
  `issues/` (not the stale narrative above). See its own Answer and the
  Decisions-so-far entry above — notably the `type`→`kind` forced rename
  and the `list`-not-`query` frontier-primitive finding. Resolving it
  unblocks [26 — Migrate `.scratch/` to the `tickets`
  Category](issues/26-migrate-scratch-to-tickets-category.md) directly (its
  only blocker). True frontier as of this session (excluding `11`, claimed
...
- **Ticket 28 resolved, 2026-08-12.** Picked up as the frontier's
  lowest-numbered unclaimed-and-unblocked ticket. Ran via a background
  research agent against primary Hugging Face Hub sources (model files,
  cards, and transformers.js's own model registry, not secondary
  write-ups); full findings published as
  [28-reranker-research.md](issues/28-reranker-research.md). Recommends
  `Xenova/ms-marco-MiniLM-L-6-v2` (22.7M params, Apache-2.0, confirmed ONNX,
  plain-BERT cross-encoder) with `mixedbread-ai/mxbai-rerank-xsmall-v1` as a
  documented backup; see its own Answer for the two real rejections worth
  remembering (`jina-reranker-v1-turbo-en`'s ONNX-exists-but-unsupported
  architecture trap, `bge-reranker-v2-m3`'s missing port and oversize).
  Resolving it unblocks [29 — Build and Pilot the Reranker Gate
  Layer](issues/29-build-pilot-reranker-gate.md) directly (its only
  blocker). True frontier as of this session (excluding `11`, claimed but
...
- **Ticket 30 resolved, 2026-08-12** — picked up as the frontier's lowest
  ...
  blocked.
- **Ticket 31 resolved, 2026-08-12** — picked up as the frontier's lowest
  id, unblocked. See its own Answer and the Decisions-so-far entry above.
  Didn't unblock anything directly (nothing else was blocked by `31`, per a
...
- **Ticket 32 resolved, 2026-08-13** — picked up as the frontier's lowest
  id, unblocked (confirmed by scanning `issues/` directly: `32`, `33`, `34`,
  `35`, `36`, `39`, `40`, `41`, `43` unclaimed/unblocked; `02`, `04`, `05`,
  `38` claimed and in progress; `03`, `42` blocked). See its own Answer and
  the Decisions-so-far entry above. Worked on `feat/2.4.0-rc2` per the prior
  session's branch instruction. Didn't unblock anything directly (no ticket
  here lists it as a blocker). True frontier as of this session: `33`, `34`,
  `35`, `36`, `39`, `40`, `41`, `43` (all unclaimed and unblocked); `02`,
  `04`, `05`, `38` claimed and in progress; `03`, `42` blocked.
- **Ticket 34 resolved, 2026-08-13** — picked up as the frontier's lowest
  ...
  above. Didn't unblock anything directly (no ticket here lists it as a
  blocker, confirmed by scanning `issues/`). True frontier as of this
  session: `35`, `36`, `39`, `40`, `41`, `43` (all unclaimed and unblocked);
  `02`, `04`, `05`, `38` claimed and in progress; `03`, `42` blocked.
- **Ticket 35 resolved, 2026-08-13** — picked up as the frontier's lowest
  id, unblocked. Worked on `feat/2.4.0-rc2` per the standing branch
  instruction above. See its own Answer and the Decisions-so-far entry
  above. Didn't unblock anything directly (no ticket here lists it as a
  blocker, confirmed by scanning `issues/`). True frontier as of this
...
- **Ticket 36 resolved, 2026-08-13** — picked up as the frontier's lowest
  ...
  claimed and in progress; `03`, `42` blocked.
- **Ticket 39 resolved, 2026-08-13** — picked up as the frontier's lowest
  id, unblocked. Worked on `feat/2.4.0-rc2` per the standing branch
  instruction above. See its own Answer and the Decisions-so-far entry
  above. Found and chartered [44](issues/44-sqlite-migration-race-multiprocess.md)
  along the way (a real, unrelated, pre-existing SQLite migration race —
  chartered, not fixed, per the one-ticket-per-session rule) rather than
  silently leaving it as an unticketed observation. Didn't unblock anything
  directly (no ticket here lists it as a blocker, confirmed by scanning
...
- [32 — CI Architecture-Drift Gate](issues/32-ci-architecture-drift-gate.md) — added an "Architecture drift check" step to `publish.yml`'s `build-and-test` job, right after `npm test` (so `dist/cli.js` already exists). No CLI code change needed: `neuron scan --check` already implements exactly the 0/1/2 exit-code contract, exhaustively unit-tested. The step branches on exit code to post differentiated `::error::` guidance — real drift vs. an incomparable (parser-fidelity-mismatched) baseline are told apart, matching the exit-code contract's own documented distinction — both point the contributor at `neuron scan` locally. Read-only, no `.neuron/` write-back, per F1b's explicit rejection. Live-verified against this repo's own real `dist/cli.js` and committed baseline (exit 0, in sync), not just fixtures.
- [31 — `neuron init` Silently Onboards Every Detected Harness, Not Just the One In Use](issues/31-unscoped-init-silently-onboards-harnesses.md) — root cause was two independent, unrelated detection paths (`CopilotAdapter.detect()`, gating hooks only; the `HARNESSES`-table check, gating the skill-dir copy and `AGENTS.md` write that this repo's bug report actually hit) both keyed off bare `.github/` existence, which GitHub creates constantly for CI/issue templates unrelated to Copilot — unlike `.claude/`/`.codex/`/`.cursor/`, which have no comparable unrelated creator. Fixed by narrowing the *signal* rather than the *policy*: `harnesses.json`'s `github` entry gained a real, GitHub-documented `.github/copilot-instructions.md` marker; a new `isHarnessPresent()` checks it (falling back to bare `base` for the other three, unchanged) OR an "already onboarded by neuron before" signal so refreshing a real Copilot install keeps working; `CopilotAdapter.detect()` now delegates to the same check instead of duplicating it. Added a visibility net for the underlying "no way to tell from the run's own output" complaint generally: a stderr note plus a `harnesses.detected`/`newlyOnboarded` JSON field whenever a harness is onboarded for the first time. Found and deliberately left unfixed: `--harness` was already documented as unable to force-wire an undetected harness, but never actually gated the `AGENTS.md`/skill-dir writes at all (only hook install) — a separate, differently-shaped ticket if pursued. `npm test` 693/693, `tsc` clean; live-verified against a real fixture tree and against this repo's own install.
