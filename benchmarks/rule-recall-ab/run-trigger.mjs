#!/usr/bin/env node
/**
 * Trigger-mode orchestrator for ticket 7, Map — MCP Server & Setup/Onboarding
 * Skill Split. Fourth iteration after three ceiling effects (run.mjs,
 * run-hard.mjs, run-action.mjs all hit `control`=100%). Re-reading
 * write-compliance-ab's own findings doc
 * (docs/design/write-compliance/nudge-ab-findings.md) surfaced the actual
 * mechanism its hard mode exploited: the rule is CONDITIONAL on an earlier
 * triggering event ("whenever a failing test gets fixed, record it
 * immediately") — not an unconditional "before you finish, always do X"
 * checklist item, which puts the obligation at exactly the moment
 * (`finish_task`) a model naturally reviews what's left to do, the easiest
 * possible moment to satisfy it. `run-action.mjs`'s rule was unconditional;
 * this one fixes that with `fixtures.mjs`'s `TRIGGER_RULE_TEXT`, which fires
 * at the SAME early moment as the task's own step-1 fix — genuine episodic
 * recall required by session end, not a checklist item. Everything else —
 * three arms, the multi-step task, the style-guide dilution, cost/
 * concurrency plumbing, grading (`sessionWasLogged` — final-file-state,
 * timing-agnostic) — is identical to run-action.mjs.
 *
 * Usage:
 *   node benchmarks/rule-recall-ab/run-trigger.mjs --dry-run
 *   node benchmarks/rule-recall-ab/run-trigger.mjs                     # pilot: 6 sessions
 *   node benchmarks/rule-recall-ab/run-trigger.mjs --k=5 --tasks=stats-multi-step,text-multi-step
 *
...
import { buildFixture, cleanupFixture, systemNoteForArmTrigger, taskPasses, TRIGGER_RULE_TEXT } from './fixtures.mjs';
...
const OUT_NAME = args.find(a => a.startsWith('--out='))?.split('=')[1] ?? 'trigger';
...
async function main() {
  ...
  console.log(
    `Ticket 7 TRIGGER rule-recall A/B — ${DRY_RUN ? 'DRY RUN (no API calls)' : `LIVE (${MODEL}, effort=${EFFORT}), hard cap $${HARD_CAP_USD}`}`
  );
  ...
  const results = await withConcurrency(plan, DRY_RUN ? 1 : CONCURRENCY, async ({ task, arm, sessionLabel }) => {
    ...
    const fixture = buildFixture(task, sessionLabel, withNeuron, TRIGGER_RULE_TEXT);
    fixture.systemNote = systemNoteForArmTrigger(arm);
    try {
      if (DRY_RUN) {
      ...
      const result = await runSession({
        ...
        sessionLabel,
        effort: EFFORT,
        hookInjectionText: TRIGGER_RULE_TEXT,
      });
  ...
  console.log('\n================================================================');
  console.log(' SCORECARD — rule compliance rate (.session-log written), TRIGGER suite');
  console.log('================================================================');
  for (const arm of ARMS) {
