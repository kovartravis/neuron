/**
 * Ticket 2 (Map — Cross-Referenced Recall: Scan, Git & Decisions): A/B
 * validation of `cross-encoder/nli-MiniLM2-L6-H768` against
 * `corpus-arch-diff.ts` — decisions/architecture premises paired against
 * `neuron scan --diff`-shaped hypotheses, rather than Ticket 8's free-form
 * decision-vs-decision pairs. Same scoring/sweep methodology as
 * `run-ab.ts`, against the new corpus, writing to its own raw-scores file
 * so Ticket 8's original run isn't overwritten.
 *
 * Run: npx tsx benchmarks/nli-polarity-ab/run-ab-arch-diff.ts
 */
import fs from 'node:fs';
import path from 'node:path';
import os from 'node:os';
import { CORPUS, type PolarityLabel } from './corpus-arch-diff.js';
...
async function main() {
  ...
  const rawOut = { timestamp: new Date().toISOString(), model: MODEL_ID, corpus: 'corpus-arch-diff', results };
  const rawPath = path.join(import.meta.dirname, 'raw-scores-arch-diff.json');
