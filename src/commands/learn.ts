...
export async function handleLearnCommand(
  ...
  if (subCommand === 'add') {
    ...
    const res = await memory.transact([{ op: 'upsert', category: 'learning', content, tags: options.tags, importance: options.importance, scope: options.scope }]);
    ...
    const results = await memory.query({ text: queryText, categories: ['learning'], limit: options.limit, scopes: options.scopes });
    console.log(JSON.stringify({ results, project: projectName, query: queryText }));
  } else if (subCommand === 'list') {
    const results = await memory.query({ categories: ['learning'], limit: options.limit });
    ...
    const res = await memory.transact([{ op: 'delete', category: 'learning', id }]);
    console.log(JSON.stringify(res[0]));
    ...
    const res = await memory.transact([{ op: 'update', category: 'learning', id, content, tags: options.tags, importance: options.importance, scope: options.scope }]);
    console.log(JSON.stringify(res[0]));
  } else {
